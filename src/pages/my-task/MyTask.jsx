...
const MyTask = () => {
  ...
  const onlyQCTask = allTask?.filter((task) => tasksWithQcUser?.includes(task.id) && task.status === "checklist");




  const qcCompleteTask = allTask?.filter(task => task.status === "qc_complete");
