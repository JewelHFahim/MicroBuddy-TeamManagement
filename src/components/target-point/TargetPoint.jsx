  ...
  return (
    <section className="mt-10 target-point w-[590px] font-Manrope h-[94px] bg-white rounded-[9px] px-[6px] py-[13px] flex justify-between items-center">
      ...
      <div className="w-[100px]">
        <div className="flex justify-between items-center text-[#7C8DB5] text-[14px] font-semibold">
          <p>Progress: </p>

          <p className="text-[#363F5E]">
            {(
              (Number(userDetails?.score) /
                Number(
                  targetPointDetail?.[0]?.target === 0
                    ? 1
                    : targetPointDetail?.[0]?.target
                )) *
              100
            ).toFixed(0)}
            %
        ...
        <div className="h-[8px] bg-[#E9EAEC] rounded-[9px]">
          <div
            className={`bg-[#307EF3] h-[8px] rounded-[9px] mt-[13px]`}
            style={{
              width:
                (Number(userDetails?.score) /
                  Number(targetPointDetail?.[0]?.target)) *
                100,
            }}
          ></div>
        </div>
